...
class PlayerTotalStats {
    ...
    init(statsDocument : DocumentSnapshot) {
        ...
        totalEFF = statsDocument.get("EFF") as! Int
        avgEFF = gamesPlayed == 0 ? 0.0 : Double(totalEFF)/Double(gamesPlayed)
        
        var minutes = Int(floor(statsDocument.get("MINS") as! Double))
        var seconds = Int(round((statsDocument.get("MINS") as! Double).truncatingRemainder(dividingBy: 1) * 60))
        totalMINS = (minutes == 0 && seconds == 0) ? "0:0" : (seconds < 10 ? "\(minutes):0\(seconds)" : "\(minutes):\(seconds)")
        
        let avgm = gamesPlayed == 0 ? 0.0 : Double(statsDocument.get("MINS") as! Double)/Double(gamesPlayed)
        minutes = Int(floor(avgm))
        seconds = Int(round(avgm.truncatingRemainder(dividingBy: 1) * 60))
        avgMINS = (minutes == 0 && seconds == 0) ? "0:0" : (seconds < 10 ? "\(minutes):0\(seconds)" : "\(minutes):\(seconds)")
    }
